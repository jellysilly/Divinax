  ...
  return (
    <div className="cols wrap-sm">
      ...
      {ch ? (
        ...
        <Panel className="fill grow" title="Карточка персонажа">
          <div className="empty">Создайте персонажа кнопкой «+» или импортируйте PNG/JSON-карточку.</div>
        </Panel>
      )}
    </div>
  );
      ...
      <div className="body scroll grow" style={{ paddingRight: 4 }}>
        <div className="char-hero">
          <div className="hero-avatar" style={{ width: 116, height: 116, margin: 12 }}>
            <div className="ring" />
            <button type="button" onClick={() => void changeAvatar()} title="Сменить аватар" style={{ borderRadius: '50%' }}>
          ...
          <div className="col grow" style={{ gap: 12, minWidth: 0, width: '100%' }}>
            <div className="row wrap" style={{ alignItems: 'flex-end', gap: 14 }}>
              <Field label="Имя персонажа" className="grow" style={{ minWidth: 220 }}>
              ...
              <div className="row" style={{ paddingBottom: 4 }}>
                <IconBtn size="lg" active={ch.fav} icon={<StarIcon size={16} fill={ch.fav ? 'currentColor' : 'none'} />} label={ch.fav ? 'Убрать из избранного' : 'В избранное'} onClick={() => set({ fav: !ch.fav })} />
                <IconBtn size="lg" icon={<MessageCircle size={16} />} label="Открыть чат" onClick={() => openOwner('char', ch.id)} />
                <IconBtn
                  size="lg"
                  icon={<BookMarked size={16} />}
                  label={book ? `Лорбук: ${book.name}` : 'Создать лорбук персонажа'}
                  active={Boolean(book)}
                  onClick={() => {
                    if (book) {
                    ...
                    const now = Date.now();
                    const lb = { id: uid(), name: `${ch.name} — лорбук`, entries: [], createdAt: now, updatedAt: now };
                    upsertLorebook(lb);
                ...
                <IconBtn
                  ...
                  label="Дублировать"
                  onClick={() => {
                    const c = { ...structuredClone(ch), id: uid(), name: ch.name + ' (копия)', createdAt: Date.now() };
                    upsertCharacter(c);
                    setState({ editingCharId: c.id });
                  }}
                />
                <IconBtn size="lg" icon={<Download size={16} />} label="Экспорт (PNG / JSON)" onClick={() => openModal('exportChar', ch.id)} />
                <IconBtn
                  size="lg"
                  className="danger"
                  icon={<Trash2 size={16} />}
                  ...
                  onClick={() => {
                    if (!confirm(`Удалить «${ch.name}» и все его чаты?`)) return;
                    setState((s) => {
            ...
            <TagInput values={ch.tags} onChange={(v) => set({ tags: v })} placeholder="+ тег" suggestions={allTags} />
          </div>
        </div>
        <div className="row wrap" style={{ justifyContent: 'space-between' }}>
          <span className="sub">
            Всего <b style={{ color: 'var(--text)' }}>{fmtNum(total)}</b> {plural(total, 'токен', 'токена', 'токенов')} · постоянно в контексте{' '}
            <b style={{ color: 'var(--text)' }}>{fmtNum(permanent)}</b>
          </span>
          <div className="row wrap">
            <button type="button" className="btn sm" onClick={() => openModal('charAdvanced', ch.id)}>
              <SlidersHorizontal size={15} /> Расширенные поля
            </button>
            <button type="button" className="btn sm" onClick={() => openModal('creatorNotes', ch.id)}>
              <FileText size={15} /> Заметки создателя
            </button>
          </div>
        </div>
        <div className="grid2" style={{ alignItems: 'start' }}>
          <div className="col" style={{ gap: 16 }}>
            <Field label="Описание">
              <LazyTextarea className="textarea serif" rows={8} value={ch.description} onCommit={(v) => set({ description: v })} placeholder="Внешность, характер, история, манера речи…" />
            </Field>
            <Field
              label={
                <span className="row" style={{ justifyContent: 'space-between', width: '100%' }}>
                  <span>{gi === 0 ? 'Первое сообщение' : `Приветствие ${gi + 1}`}</span>
                  <span className="row" style={{ gap: 6, letterSpacing: 0 }}>
                    <IconBtn size="sm" icon={<ChevronLeft size={14} />} label="Предыдущее" disabled={gi === 0} onClick={() => setGreet(gi - 1)} />
                    <span className="tabular" style={{ fontSize: 12.5, fontWeight: 500 }}>
                      {gi + 1} / {greetings.length}
                    </span>
                    <IconBtn size="sm" icon={<ChevronRight size={14} />} label="Следующее" disabled={gi >= greetings.length - 1} onClick={() => setGreet(gi + 1)} />
                    <IconBtn
                      size="sm"
                      icon={<Plus size={14} />}
                      label="Добавить приветствие"
                      onClick={() => {
          ...
          <div className="col" style={{ gap: 16 }}>
            ...
            <Field label="Примеры диалогов" hint="Разделяйте примеры строкой <START>">
...
export async function exportCharacter(ch: Character, kind: 'png' | 'json') {
  ...
  if (kind === 'json') download(`${safeName(ch.name)}.json`, JSON.stringify(characterToJson(ch, book), null, 2));
    ...
    try {
      ...
      toast('Не удалось собрать PNG: ' + (e as Error).message, 'error');
    }
  }
}
